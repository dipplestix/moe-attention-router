...
class MoEGate(nn.Module):
    def __init__(self, config):
        ...
        self.gating_dim = int(config.hidden_size/config.num_attention_heads)
...
class DeepseekAttention(nn.Module):
    ...
    def __init__(self, config: DeepseekConfig, layer_idx: Optional[int] = None):
        ...
        self.q_gate = nn.Linear(self.hidden_size, self.head_dim, bias=config.attention_bias)
        self.k_gate = nn.Linear(self.hidden_size, self.head_dim, bias=config.attention_bias)
        self.v_gate = nn.Linear(self.hidden_size, self.head_dim, bias=config.attention_bias)
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_gate = self.q_gate(hidden_states)
        key_gate = self.k_gate(hidden_states)
        value_gate = self.v_gate(hidden_states)
        gate_output = F.scaled_dot_product_attention(query_gate, key_gate, value_gate)

        return attn_output, attn_weights, past_key_value, gate_output
